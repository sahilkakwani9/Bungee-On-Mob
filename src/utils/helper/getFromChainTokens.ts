...
async function getFromChainTokens(
  fromChainId: number,
  toChainId: number
): Promise<TokenResult> {
  try {
    const response = await fetch(
      `${SOCKET_API_URL}/token-lists/from-token-list?fromChainId=${fromChainId}&toChainId=${toChainId}&singleTxOnly=true`,
      {
        method: "GET",
        headers: {
...
export default getFromChainTokens;
